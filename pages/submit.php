...
if ($_POST) {
    ...
    foreach ($m as $daym) {
		...
		natural join member natural join time where schedule.member_id='$member' and schedule.time_id='$daym' and schedule.day='m'")or die(mysqli_error($con));
        ...
        $error_t = "<span class='text-danger'>
			<table class ='table table-stripped table-bordered' width='100%'>
				<tr class='bg-danger'>
					...
					<td>$time1</td>
					<td>$member1</td>
					<td class='text-red'><b>Not Free</b></td>
				</tr>
				</span>
			</table> 
            <script>
     ...
     toastr.error('A duplicate schedule! <b>$cysc at Monday $timec </b> is not idle.')</script> ";
        ...
        if (($count_t == 0) and ( $count_r == 0) and ( $count_c == 0)) {
            mysqli_query($con, "INSERT INTO schedule(time_id,day,member_id,subject_code,cys,room,remarks,settings_id,encoded_by,dept_id)
				VALUES('$daym','m','$member','$subject','$cys','$room','$remarks','$set_id','$program','$_SESSION[dept_id]')")or die(mysqli_error());
    ...
    foreach ($w as $daym) {
     ...
     toastr.error('A duplicate schedule! <b>$cysc at Wednesday $timec </b> is not idle.')</script> ";
        ...
        $queryt = mysqli_query($con, "select * from member where member_id='$member'")or die(mysqli_error($con));
        $rowt = mysqli_fetch_array($queryt);
        $membert = $rowt['member_first'] . " " . $row['member_last'];
        ...
        if (($count_t == 0) and ( $count_r == 0) and ( $count_c == 0)) {
            mysqli_query($con, "INSERT INTO schedule(time_id,day,member_id,subject_code,cys,room,remarks,settings_id,encoded_by,dept_id)
				VALUES('$daym','w','$member','$subject','$cys','$room','$remarks','$set_id','$program','$_SESSION[dept_id]')")or die(mysqli_error());
    ...
    foreach ($f as $daym) {
     ...
     toastr.error('A duplicate schedule! <b>$cysc at Friday $timec </b> is not idle.')</script> ";
        ...
        if (($count_t == 0) and ( $count_r == 0) and ( $count_c == 0)) {
            mysqli_query($con, "INSERT INTO schedule(time_id,day,member_id,subject_code,cys,room,remarks,settings_id,encoded_by,dept_id)
				VALUES('$daym','f','$member','$subject','$cys','$room','$remarks','$set_id','$program','$_SESSION[dept_id]')")or die(mysqli_error());
    ...
    foreach ($t as $daym) {
     ...
     toastr.error('A duplicate schedule! <b>$cysc at Tuesday $timec </b> is not idle.')</script> ";
        ...
        if (($count_t == 0) and ( $count_r == 0) and ( $count_c == 0)) {
            mysqli_query($con, "INSERT INTO schedule(time_id,day,member_id,subject_code,cys,room,remarks,settings_id,encoded_by,dept_id)
				VALUES('$daym','t','$member','$subject','$cys','$room','$remarks','$set_id','$program','$_SESSION[dept_id]')")or die(mysqli_error($con));
    ...
    foreach ($th as $daym) {
     ...
     toastr.error('A duplicate schedule! <b>$cysc at Thursday $timec </b> is not idle.')</script> ";
        ...
        if (($count_t == 0) and ( $count_r == 0) and ( $count_c == 0)) {
            mysqli_query($con, "INSERT INTO schedule(time_id,day,member_id,subject_code,cys,room,remarks,settings_id,encoded_by,dept_id)
				VALUES('$daym','th','$member','$subject','$cys','$room','$remarks','$set_id','$program','$_SESSION[dept_id]')")or die(mysqli_error());
    ...
    foreach ($sa as $daym) {
        //check conflict for member
        $query_member = mysqli_query($con, "select *,COUNT(*) as count from schedule
		natural join member natural join time where member_id='$member' and schedule.time_id='$daym' and day='sa'")or die(mysqli_error($con));
        $row = mysqli_fetch_array($query_member);
        $count_t = $row['count'];
        $time1 = date("h:i a", strtotime($row['time_start'])) . "-" . date("h:i a", strtotime($row['time_end']));
        $member1 = $row['member_first'] . ", " . $row['member_last'];
     ...
     toastr.error('A duplicate schedule! <b>$member1 at Saturday $time1</b> is not idle.')</script> ";
        ...
        //check conflict for room
        $query_room = mysqli_query($con, "select *,COUNT(*) as count from schedule
		natural join member natural join time where room='$room' and schedule.time_id='$daym' and day='sa'")or die(mysqli_error($con));
        $rowr = mysqli_fetch_array($query_room);
        $count_r = $rowr['count'];
        $timer = date("h:i a", strtotime($rowr['time_start'])) . "-" . date("h:i a", strtotime($rowr['time_end']));
        $roomr = $rowr['room'];
     ...
     toastr.error('A duplicate schedule! <b>Room $roomr at Saturday $timer</b> is not idle.')</script> ";
        //check conflict for class
        $query_class = mysqli_query($con, "select *,COUNT(*) as count from schedule
		natural join member natural join time where cys='$cys' and schedule.time_id='$daym' and day='sa'")or die(mysqli_error($con));
        $rowc = mysqli_fetch_array($query_class);
        $count_c = $rowc['count'];
        $cysc = $rowc['cys'];
        $timec = date("h:i a", strtotime($rowc['time_start'])) . "-" . date("h:i a", strtotime($rowc['time_end']));
     ...
     toastr.error('A duplicate schedule! <b>$cysc at Saturday $timec </b> is not idle.')</script> ";
        ...
        if (($count_t == 0) and ( $count_r == 0) and ( $count_c == 0)) {
            mysqli_query($con, "INSERT INTO schedule(time_id,day,member_id,subject_code,cys,room,remarks,settings_id,encoded_by,dept_id)
				VALUES('$daym','sa','$member','$subject','$cys','$room','$remarks','$set_id','$program','$_SESSION[dept_id]')")or die(mysqli_error());
    ...
    foreach ($su as $daym) {
        //check conflict for member
        $query_member = mysqli_query($con, "select *,COUNT(*) as count from schedule
		natural join member natural join time where member_id='$member' and schedule.time_id='$daym' and day='su'")or die(mysqli_error($con));
        $row = mysqli_fetch_array($query_member);
        $count_t = $row['count'];
        $time1 = date("h:i a", strtotime($row['time_start'])) . "-" . date("h:i a", strtotime($row['time_end']));
        $member1 = $row['member_first'] . ", " . $row['member_last'];
     ...
     toastr.error('A duplicate schedule! <b>$member1 at Sunday $time1</b> is not idle.')</script> ";
        ...
        //check conflict for room
        $query_room = mysqli_query($con, "select *,COUNT(*) as count from schedule
		natural join member natural join time where room='$room' and schedule.time_id='$daym' and day='su'")or die(mysqli_error($con));
        $rowr = mysqli_fetch_array($query_room);
        $count_r = $rowr['count'];
        $timer = date("h:i a", strtotime($rowr['time_start'])) . "-" . date("h:i a", strtotime($rowr['time_end']));
        $roomr = $rowr['room'];
     ...
     toastr.error('A duplicate schedule! <b>Room $roomr at Sunday $timer</b> is not idle.')</script> ";
        //check conflict for class
        $query_class = mysqli_query($con, "select *,COUNT(*) as count from schedule
		natural join member natural join time where cys='$cys' and schedule.time_id='$daym' and day='su'")or die(mysqli_error($con));
        $rowc = mysqli_fetch_array($query_class);
        $count_c = $rowc['count'];
        $cysc = $rowc['cys'];
        $timec = date("h:i a", strtotime($rowc['time_start'])) . "-" . date("h:i a", strtotime($rowc['time_end']));
     ...
     toastr.error('A duplicate schedule! <b>$cysc at Sunday $timec </b> is not idle.')</script> ";
        ...
        if (($count_t == 0) and ( $count_r == 0) and ( $count_c == 0)) {
            mysqli_query($con, "INSERT INTO schedule(time_id,day,member_id,subject_code,cys,room,remarks,settings_id,encoded_by,dept_id)
				VALUES('$daym','su','$member','$subject','$cys','$room','$remarks','$set_id','$program','$_SESSION[dept_id]')")or die(mysqli_error());
