...
if (isset($_POST['login'])) {
    ...
    $query = mysqli_query($con, "select * from member where username='$user' and password='$pass' and status='faculty'")or die(mysqli_error($con));
    ...
    $dept_id = $row['dept_id'];
    if ($counter == 0) {
        echo "<script type='text/javascript'>alert('IIInvalid Username or Password!');
          document.location='faculty_dean_index.php'</script>";
    } else {
